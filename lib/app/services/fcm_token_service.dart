...
class FcmTokenService extends GetxService {
  ...
  Future<void> updateTokenOnServer(String? token) async {
    try {
      final userId = _preferences.getString(Keys.userId);
      if (userId == null || userId.isEmpty || token == null || token.isEmpty) {
        DebugPoint.log('⚠️ Cannot update token: userId or token is empty');
        return;
      }

      // Update token via profile update API
      final body = {
        'fcmToken': token,
      };

      // final response = updateUserFcmToken(body);
      
      // if (response != null) {
      //   DebugPoint.log('✅ FCM Token updated on server');
      // } else {
      //   DebugPoint.log('❌ Failed to update FCM token on server');
      // }
    } catch (e) {
